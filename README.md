...
- **Kelas & Jadwal** (menu **Kelas**): tab **Kelas** (tambah/edit/hapus + **checkbox hapus massal** + **Import/Export CSV** + template, kolom Nama Kelas/Tingkat/Jurusan/Ruang, plus tombol **printer** di tiap kelas untuk **cetak kartu QR absen** semua siswanya), tab **Jurusan** (tambah/edit/hapus dengan kolom kode), tab **Mapel** (tambah/edit/hapus + **checkbox hapus massal** + **Import/Export CSV** + template), tab **Jadwal** (tambah/edit/hapus per jadwal).
...
---
- **Perbaikan bug "Pulang Awal" di jam 15:24** 🐛⏰ — absen pulang yang dilakukan SETELAH jam pulang sekolah (mis. 15:24 WIB) kini **tidak lagi ditandai "Pulang Awal"**. Penyebabnya: server (container VPS) berjalan di zona UTC sehingga jam 15:24 WIB terbaca 08:24 dan dianggap sebelum jam pulang. Perhitungan "pulang awal" sekarang memakai **waktu lokal sekolah (WIB)**. Nilai jam/menit di pengaturan juga disanitasi — isian tidak valid seperti "7.10" / "10.1" otomatis dinormalisasi (mis. 10.1 → ikut jam pulang 15:00).
- **Form pengaturan jam absen pakai input HH:MM** 🕐 — empat aturan (batas terlambat, batas akhir datang, jam pulang sekolah, mulai dihitung pulang awal) kini memakai **satu kolom waktu "HH:MM"** (picker jam bawaan) menggantikan dua kolom angka "Jam" + "Menit" yang rawan salah isi seperti "7.10". Simpan sekali lagi setelah update untuk menyesuaikan jamnya.
- **Jadwal Hari Ini tampil di beranda siswa** 🗓️ — jadwal pelajaran yang di-set admin (menu Kelas → Jadwal) kini **benar-benar ditampilkan untuk siswa** di beranda (kartu jadwal hari ini: jam, mapel, guru, ruang). Sebelumnya data jadwal sudah dikirim API tapi tidak pernah dirender di halaman siswa, jadi jadwal yang sudah diatur tidak terlihat.
- **Absen wajah: kamera langsung menyala, default Absen Datang** 📸 — halaman absen wajah kini **langsung membuka kamera dan otomatis memindai tanpa perlu menekan tombol**, dengan default **Absen Datang**. Jika kamera gagal menyala (mis. izin belum diberikan atau WebView menahan video sampai disentuh), muncul tombol **"Mulai Kamera"** untuk mencoba lagi, dan `video.play()` yang ditahan otomatis dicoba ulang saat layar disentuh — mengatasi kasus kamera tidak tampil sampai tombol Datang/Pulang diklik.
...
- **Absen datang & pulang: aturan jam baru** ⏰ — absen **datang** yang diulang (wajah/QR) tidak lagi error dan **tidak menimpa** catatan: jam datang **paling awal** yang tercatat (scan 06:30, lalu 06:45 / 07:10 tetap 06:30). Absen **pulang** yang diulang **menggantikan** catatan lama: jam pulang **terbaru** yang tercatat (absen pulang 14:00 lalu 15:01 → yang tercatat 15:01; penanda "Pulang Awal" ikut dihitung ulang).
- **Halaman QR siswa disederhanakan** 📱 — tombol **"Absen Sekarang"** di bawah QR **dihapus** (siswa tidak bisa absen sendiri lewat tombol) dan tab Datang/Pulang disembunyikan untuk siswa — QR hanya untuk **ditunjukkan ke kamera gerbang**.
- **Kartu "Kehadiran Saya" menampilkan kejadian terakhir** 🏠 — beranda siswa kini menampilkan **jam absen terakhir**: jam **pulang** bila sudah pulang (label "Sudah pulang" / "Pulang Awal"), selain itu jam **datang** + status. Notifikasi absen datang/pulang juga dikirim lebih andal (kegagalan notifikasi tidak menggagalkan absen yang sudah tercatat).
...
- **Menu Rekap Absensi di Laporan** 📋 — tab "Rekap Absensi" baru di halaman Laporan (untuk admin, superadmin, petugas piket, wali kelas): menampilkan tabel spreadsheet per siswa dengan kolom **No, Kelas, Nama**, akumulasi **S/I/A/D/P** per semester, akumulasi **30 hari terakhir**, dan kolom harian warna-warni (H=Hadir hijau, S=Sakit kuning, I=Izin biru, A=Alpha merah, D=Dinas ungu). Tabel bisa di-scroll horizontal, kolom No/Kelas/Nama sticky di kiri. Hari ini ditandai border kuning.
- **Menu Riwayat untuk admin & superadmin** 🗂️ — akun Admin/TU dan Super Admin kini memiliki menu **Riwayat** (di sidebar desktop, drawer mobile, dan nav bawah): menampilkan riwayat absensi **semua kelas** dengan filter per kelas, rekap bulanan per status, dan tombol hapus bersih catatan absensi (sesuai permintaan sebelumnya).

- **Tampilan login desktop baru ala split-screen** 🖥️ — halaman login di desktop kini tampil dua kolom: panel branding gelap (teal) di kiri berisi logo, nama aplikasi, tagline, headline, daftar fitur, dan footer; form login di kanan dengan heading "Masuk ke Panel", tab peran (Guru & Staff / Admin & TU / Siswa / Orang Tua), ikon mata untuk lihat/sembunyikan password, dan catatan keamanan. Versi mobile tetap memakai tampilan lama yang terpusat. Jika logo sekolah & background login diunggah di Pengaturan, keduanya otomatis dipakai di halaman login.
...
- **Kartu Kehadiran per Kelas bisa diklik** 👆 — di dashboard (admin & petugas piket), kartu **"Kehadiran per Kelas"** kini bisa **diklik untuk langsung membuka detail absensi kelas** tersebut (daftar siswa, status, filter Hadir/Belum, pemilih tanggal, dan validasi status) — ditandai ikon panah dan hint "Klik kartu kelas untuk membuka detail absensi".
- **Dashboard Petugas Piket ala Admin** 📊 — beranda guru piket kini menampilkan **statistik absensi seluruh sekolah** seperti dashboard admin: kartu Kehadiran Hari Ini / Hadir / Terlambat / Izin-Sakit / Belum Hadir / Total Siswa, grafik pie, **Kedatangan Terbaru (live)**, **Kehadiran per Kelas**, dan daftar **Belum Melakukan Absensi** — **realtime** tanpa reload. Bagian **"Sesi Mengajar" dihilangkan** untuk akun piket (kartu aksi Scan Gerbang / Absen Manual / Persetujuan Izin / Riwayat tetap ada).
- **Status Dispensasi** 🛡️ — status absen baru **"Dispensasi"** tersedia di Absen Manual / koreksi (admin, piket, wali kelas) dan **ditampilkan di Riwayat** (badge teal + kartu rekap bulanan), laporan harian/bulanan (ringkasan, grafik, rekap per kelas), export PDF/Excel, dan dashboard. Siswa dengan dispensasi masuk hitungan "izin/sakit" di rekap.
...
- **Batas waktu absen datang & pulang cepat di Pengaturan** ⏰ — dua aturan baru di **Pengaturan → Aturan Absensi**:
  ...
  - **Mulai dihitung Pulang Awal** (default = ikut jam pulang sekolah): siswa yang absen pulang **sebelum** jam ini ditandai **"Pulang Awal"** di riwayat — bisa diatur terpisah dari jam pulang sekolah bila sekolah ingin batas yang berbeda.
  - Pengaturan baru ini juga bisa diset lewat `.env` (`CHECK_IN_DEADLINE_HOUR/MINUTE`).
- **Kartu QR kecil versi "QR Besar"** 🔍 — di halaman Cetak Kartu QR, setelah memilih **Kecil ID (2×3,5 cm)** kini ada pilihan **QR Standar** (QR ±1,7 cm kiri + NISN & nama kanan, kartu 2×3,5 cm) atau **QR Besar** (QR **±2,3 cm** di tengah + NISN & nama di bawahnya, kartu 3,5×3,1 cm). QR Besar **±34% lebih panjang / ±79% lebih luas** — jauh lebih mudah discan kamera gerbang dari jarak lebih jauh, sambil tetap hemat kertas (berjejer rapat di A4). QR juga di-render lebih tajam (320 px) untuk kedua ukuran.
- **Deteksi transparansi logo otomatis** 🧊 — saat upload logo di Pengaturan, sistem otomatis memeriksa apakah gambar punya **background transparan** (ada piksel tembus pandang, termasuk pinggiran anti-alias). Logo transparan **tetap disimpan sebagai PNG** sehingga transparansinya bening — tidak lagi berubah jadi **kotak putih** di kop kartu QR, login, dan PWA. Logo tanpa transparansi tetap dikompresi ke JPEG yang lebih kecil (kualitas 0.88). Notifikasi saat upload memberi tahu apakah logo transparan terdeteksi.
- **Upload Logo Sekolah di Pengaturan** 🏫🖼️ — kartu **Identitas Aplikasi** di halaman Pengaturan kini punya **form upload logo** dengan **pratinjau langsung**: pilih file (JPG/PNG/WEBP, maks 5 MB) → otomatis dikompresi ringan (512 px, kualitas 0.88) → diunggah & langsung tampil. Tombol **"Hapus logo"** mengembalikan ke logo bawaan. Logo dipakai otomatis di **kop kartu QR**, halaman **login**, dan seluruh aplikasi — jadi kop kartu QR memakai **logo asli sekolah**. Setiap upload tercatat di Audit Log (`LOGO_UPDATED`), file logo lama dibersihkan otomatis saat diganti/dihapus, dan hanya admin/superadmin yang bisa mengubahnya (dikunci di backend).
- **Aksi Edit & Hapus di tab Jurusan dan Jadwal** ✏️🗑️ — tab **Jurusan** kini punya tombol **edit** (ubah nama/kode) dan **hapus permanen** (kelas & siswa yang memakai jurusan itu dikosongkan jurusannya) di setiap kartu, plus kolom **Kode** saat menambah jurusan. Tab **Jadwal** kini punya tombol **edit** di setiap jadwal (form sama seperti tambah, terisi otomatis) dan tombol **hapus** dengan konfirmasi. Semua perubahan tercatat di Audit Log (`MAJOR_UPDATED` / `MAJOR_DELETED` / `SCHEDULE_UPDATED` / `SCHEDULE_DELETED`).
- **Cetak Kartu QR Absen per Kelas** 🖨️ — tombol **printer** di setiap kartu kelas (menu **Kelas**) dan tombol **Kartu QR** di menu **Siswa** membuka halaman cetak: pilih kelas → kartu QR absen **semua siswa** tampil dalam grid, siap diprint A4 (kartu anti-terpotong antar halaman). Ada **2 ukuran**: **Kartu Besar** (QR + nama + NISN + kelas) dan **Kartu Kecil ID 2×3,5 cm** (QR kiri ±1,7 cm + NISN & nama kanan) yang dicetak rapat di kertas label/kartu A4 — puluhan kartu per lembar, tinggal dipotong & ditempel di kartu siswa. Setiap lembar kartu dilengkapi **kop resmi** (logo aplikasi/sekolah + nama sekolah dari Pengaturan + tulisan "KARTU ABSENSI SISWA — QR CODE"). QR memakai token **student-card** yang berlaku ±1 tahun — siswa cukup menunjukkan kartunya ke kamera gerbang untuk absen, dan kartu bisa dicetak ulang kapan saja tanpa daftar ulang. Wali kelas hanya bisa mencetak kelasnya sendiri (dikunci di backend); piket/admin/superadmin semua kelas. Menu **Kartu QR** juga tersedia di sidebar/drawer ☰ Petugas Piket & Wali Kelas.
...
- **Layout tanda tangan PDF** — Kepala Sekolah di kiri, Petugas Piket di kanan, format resmi sekolah.
