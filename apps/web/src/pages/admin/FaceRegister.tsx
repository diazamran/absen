...
export default function FaceRegister() {
  ...
  const toggleAllRegistered = () => {
    const filtered = regClassFilter ? registeredFaces?.filter((r) => r.className === regClassFilter) : registeredFaces;
    if (!filtered?.length) return;
    setRegisteredSelected((prev) => (prev.size === filtered.length ? new Set() : new Set(filtered.map((r) => r.userId))));
  };
        ...
        <Card className="mb-4 border-emerald-200 bg-emerald-50/50 dark:bg-emerald-500/5">
          <div className="mb-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <p className="font-bold text-ink">Wajah Terdaftar ({filteredRegistered.length}{regClassFilter ? ` / ${registeredFaces.length}` : ''})</p>
            </div>
            <label className="flex items-center gap-2 text-xs text-muted">
              <input type="checkbox" checked={registeredSelected.size === filteredRegistered.length && filteredRegistered.length > 0} onChange={toggleAllRegistered} className="h-4 w-4 accent-[var(--primary)]" />
              Pilih semua
            </label>
          </div>
          {/* Filter per kelas */}
          {regClasses.length > 0 && (
            <div className="mb-3 flex flex-wrap items-center gap-2">
              <span className="text-xs font-semibold text-muted">Kelas:</span>
              <button
                onClick={() => { setRegClassFilter(''); setRegisteredSelected(new Set()); }}
                className={`rounded-full px-3 py-1 text-xs font-bold transition ${!regClassFilter ? 'bg-primary text-white' : 'bg-slate-100 text-muted hover:bg-slate-200 dark:bg-slate-700 dark:text-slate-300'}`}
              >
                Semua ({registeredFaces?.length ?? 0})
              </button>
              {regClasses.map((cls) => {
                const count = registeredFaces?.filter((r) => r.className === cls).length ?? 0;
                return (
                  <button
                    key={cls}
                    onClick={() => { setRegClassFilter(cls); setRegisteredSelected(new Set()); }}
                    className={`rounded-full px-3 py-1 text-xs font-bold transition ${regClassFilter === cls ? 'bg-primary text-white' : 'bg-slate-100 text-muted hover:bg-slate-200 dark:bg-slate-700 dark:text-slate-300'}`}
                  >
                    {cls} ({count})
                  </button>
                );
          ...
          {/* Quick reset per kelas */}
          {regClassFilter && filteredRegistered.length > 0 && registeredSelected.size === 0 && (
            <div className="mb-3">
              <Button variant="danger" className="!px-3 !py-1.5 text-xs" onClick={() => {
                ...
                {bulkResetRegistered.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Trash2 className="h-3.5 w-3.5" />}
                Reset Semua di Kelas {regClassFilter}
              </Button>
            </div>
          )}
