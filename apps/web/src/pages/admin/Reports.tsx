...
export default function Reports() {
  ...
  const exportCsv = () => {
    const url =
      type === 'daily'
        ? `/api/reports/export?report=daily&date=${date}&classId=${classId}`
        : `/api/reports/export?report=monthly&month=${month}&classId=${classId}`;
    ...
    fetch(url, { headers: { Authorization: `Bearer ${token}` } })
      ...
      .then((blob) => {
        ...
        a.download = `laporan_${type}_${date || month}.csv`;
        a.click();
        toast('success', 'Laporan CSV diunduh (buka dengan Excel).');
      })
  ...
  return (
    <div>
      <PageHeader
        ...
        action={<Button variant="outline" onClick={exportCsv}><Download className="h-4 w-4" /> Export CSV</Button>}
      />
